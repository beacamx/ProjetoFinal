...
Interface_Remove_Jogador::Interface_Remove_Jogador() : Interface_Menu(), caixa_de_texto1(15, sf::Color::White, false) { 
    try {
        Set_Values();
        cout << "Tamanho vetor de jogadores:" << registro_geral.jogadores.size() << endl;
    } catch (const std::exception& e) {
        cerr << "Erro ao inicializar Interface_Remove_Jogador: " << e.what() << endl;
        exit(EXIT_FAILURE);
    }
}

void Interface_Remove_Jogador::Set_Opcoes() {
    opcoes_de_escolha = {"Digitar apelido", "Play"};
}

void Interface_Remove_Jogador::Set_Image() {
     try {
        if (!image->loadFromFile("./assets/Menu/Remover_Jogador.png")) {
...
void Interface_Remove_Jogador::Definicoes_Espacamento_Janela() {
    espaco_vertical = 90.0f;
    largura_janela = 624.0f;
    altura_texto = 244.0f; 
}

void Interface_Remove_Jogador::Set_Values(){
    Set_Janela();
    Set_Image();
    Set_Opcoes();
...
void Interface_Remove_Jogador::Atualizar_Janela() {
    try {
        this->janela->clear();
...
void Interface_Remove_Jogador::Set_Aviso() {
    try {
...
void Interface_Remove_Jogador::Loop_Events(){
    sf::Event evento;
