...
\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\
para escolher um jogador do cadastro, recebendo como retorno seu ponteiro, com base em seu nome:
registro.find("Ciclano de tal");

caso o método não encontre o cadastro, retorna NULL, pois, apesar das contraindicações, nesse caso facilita a manipulação
por ser sempre 0. (diferente de nullptr)
\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\
...
para computadr vitórias e derrotas : ponteiro->win_lig4() \ ponteiro->win_Reversi() & ponteiro->lose_lig4() ponteiro->lose_Reversi();
Obs: os métodos save() e load() foram colocados públicos para permitir mais usos, já que depende apenas do estado de std::set<jogador*> jogadores.
